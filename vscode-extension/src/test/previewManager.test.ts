...
suite("PreviewManager", () => {
  test("addStream spawns process and sends AddStream command", async () => {
    ...
    await manager.addStream("stream-a", "/path/to/HogeView.swift", "iPhone16,1", "iOS-18-0");
  ...
  test("addStream with different streamId reuses process", async () => {
    ...
    await manager.addStream("stream-a", "/path/to/HogeView.swift", "iPhone16,1", "iOS-18-0");
    await manager.addStream("stream-b", "/path/to/FugaView.swift", "iPhone15,4", "iOS-17-0");
  ...
  test("sendInput writes Command JSON with streamId from message", async () => {
    ...
    await manager.addStream("stream-a", "/path/to/HogeView.swift", "iPhone16,1", "iOS-18-0");
    manager.sendInput({ type: "touchDown", streamId: "stream-a", x: 0.5, y: 0.3 });
  ...
  test("addStream propagates resolveExecutablePath error", async () => {
    ...
    await assert.rejects(
      () => manager.addStream("stream-a", "/path/to/HogeView.swift", "iPhone16,1", "iOS-18-0"),
  ...
  test("spawn args use serve mode (no source file)", async () => {
    ...
    assert.ok(!args.includes("/path/to/HogeView.swift"), "should not include source file in spawn args");
  });
  ...
  test("sendInput touchMove writes correct Command JSON", async () => {
    ...
    await manager.addStream("stream-a", "/path/to/HogeView.swift", "iPhone16,1", "iOS-18-0");
    manager.sendInput({ type: "touchMove", streamId: "stream-a", x: 0.7, y: 0.8 });
  ...
  test("sendInput touchUp writes correct Command JSON", async () => {
    ...
    await manager.addStream("stream-a", "/path/to/HogeView.swift", "iPhone16,1", "iOS-18-0");
    manager.sendInput({ type: "touchUp", streamId: "stream-a", x: 0.2, y: 0.9 });
  ...
  test("replaceAllStreams sends RemoveStream for old streams and AddStream for new", async () => {
    ...
    await manager.replaceAllStreams("stream-c", "/path/to/PiyoView.swift", "iPhone16,2", "iOS-18-1");
  ...
  test("replaceAllStreams spawns process if not running", async () => {
    ...
    await manager.replaceAllStreams("stream-a", "/path/to/HogeView.swift", "iPhone16,1", "iOS-18-0");
